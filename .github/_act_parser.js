...
const wfPath = path.resolve(__dirname, "workflows", "compare-tree.json");
const epPath = path.resolve(__dirname, "_act_event.json");
...
async function main() {
  ...
  const genWfPath = path.resolve(path.dirname(wfPath), `_act_${path.basename(wfPath)}`);
...
async function help() {
  const baseCmd = `${process.argv[0]} ${process.argv[1]}`;
  console.log(`
For handling inputs:
- ${epPath}
